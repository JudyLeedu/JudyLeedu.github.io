...
description: "循环工程是AI编码代理的未来交互范式，从“提示”到“设计循环”的转变。"
date: "2026-07-23"
icon: "☕️"
color: "amber"
---

> 原文链接，好文，推荐阅读：[Loop Engineering](https://x.com/addyosmani/status/2064127981161959567)

<img src="/images/loop-engineering.png" alt="Loop Engineering" className="w-full rounded-xl border border-slate-200 shadow-sm my-6" />

我用自己做的插件 readly 提炼的几个亮点：

> **循环工程：AI编码代理的未来交互范式**
>
> 本文提出“循环工程”是AI编码代理的下一阶段范式。核心思想是：人类不再手动提示（prompt）代理，而是设计一个自动化的循环系统来驱动代理。该系统包含自动化、工作树、技能、插件和子代理五个构建块，并依赖外部记忆。文章以Claude Code和Codex为例，展示了这些模块如何内置于现代产品中，并指出这不仅是工具层面的进步，更是工作方式的根本转变——从“人类驱动代理”到“系统驱动代理”。
>
> **01 从“提示”到“设计循环”的范式转变**
> 过去两年，使用编码代理的方式是手动写提示、读结果、再写下一个提示，代理只是人类手中的工具。现在，顶尖实践者（如Anthropic Claude Code负责人）已不再手动提示Claude，而是编写循环系统来驱动Claude，让系统自动决定下一步做什么。
>
> **02 循环工程的五大构建块**
> 一个有效的循环系统需要五个核心组件，它们已内置于Claude Code和Codex中，映射关系几乎一致：
> - 自动化：按计划自动执行发现和分类任务，无需人工干预。
> - 工作树：让多个代理并行工作时不互相干扰。
> - 技能：将项目知识（如约定、构建步骤）写入文档，代理每次运行直接读取，避免重复解释。
> - 插件与连接器：将代理接入已有工具链。
> - 子代理：一个代理负责构思，另一个负责检查，实现分离验证。
>
> **03 外部记忆是循环持续运行的关键**
> 代理每次运行都会忘记之前的上下文，因此记忆必须存储在磁盘上，而非上下文窗口中。一个简单的Markdown文件或Linear看板，记录已完成和待办事项，就能让循环系统跨会话持续工作。
>
> **04 自动化：让循环真正“循环”起来**
> 自动化是区分一次运行和持续循环的核心。Codex的自动化标签页允许设置项目、提示、频率和运行环境；Claude Code通过调度和钩子实现相同功能。两者都支持“/goal”指令——让代理持续工作直到条件满足（如“所有测试通过且代码整洁”），且检查条件由独立小模型完成，避免自我评分。
>
> **05 worktree 解决并行冲突，但人类审查仍是瓶颈**
> 当多个代理并行工作时，文件冲突成为主要失败点。Git工作树通过为每个代理提供独立工作目录来解决此问题。但作者指出，工具消除了机械碰撞，但人类的审查带宽决定了实际能并行运行多少个代理——瓶颈不在工具，而在人。
>
> **06 技能：将意图固化，避免每次循环都从零开始**
> 代理每次启动都是“冷启动”，会自信地填补意图中的空白。技能是将项目约定、构建步骤、历史教训等意图写在外部的文档（如SKILL.md），代理每次运行都读取它。没有技能，循环每次都要重新推导整个项目；有了技能，知识可以累积。
...
[@steipete](https://x.com/@steipete) recently [said](https://x.com/steipete/status/2063697162748260627) “You shouldn't be prompting coding agents anymore. You should be designing loops that prompt your agents.” Similarly,

[@bcherny](https://x.com/@bcherny), head of Claude Code at Anthropic, [said](https://x.com/rohanpaul_ai/status/2063289804708835412) “I don't prompt Claude anymore. I have loops running that prompt Claude and figure out what to do. My job is to write loops”.
