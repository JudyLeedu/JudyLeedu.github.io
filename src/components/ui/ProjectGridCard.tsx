...
export function ProjectGridCard({
  ...
  return (
    <motion.div
      ...
      className={`relative bg-white border border-transparent hover:border-blue-500/30 rounded-2xl p-6 cursor-pointer group flex shadow-[0_2px_8px_rgba(0,0,0,0.04)] hover:shadow-[0_8px_24px_rgba(0,0,0,0.08)] overflow-hidden ${
        isCore ? "md:col-span-2 flex-col md:flex-row md:items-center gap-6" : "col-span-1 flex-col justify-between gap-4"
      }`}
      onClick={onClick}
    >
      {/* 边框微光效果 */}
      ...
      <div className={`flex flex-row items-start ${isCore ? "gap-5" : "gap-4"} flex-1 z-10 w-full`}>
      ...
      <div className={`z-10 shrink-0 text-slate-300 group-hover:text-blue-500 transition-all duration-300 ${isCore ? "self-end md:self-auto group-hover:translate-x-1" : "self-end group-hover:translate-x-1"}`}>
