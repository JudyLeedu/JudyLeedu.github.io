...
export function ProjectGridCard({
  ...
  return (
    <div
      className={`bg-white border border-slate-200 cursor-pointer group transition-all duration-300 shadow-[0_1px_3px_rgba(0,0,0,0.02)] hover:shadow-[0_8px_20px_rgba(0,0,0,0.06)] hover:-translate-y-1 hover:border-blue-500/30 rounded-2xl p-6 flex flex-col justify-between overflow-hidden ${
        isCore ? "md:col-span-2" : "col-span-1"
      }`}
      onClick={onClick}
    >
      <div className={`flex flex-row items-start ${isCore ? "gap-5 sm:gap-6" : "gap-4"} z-10 w-full flex-1`}>
        ...
        <div className="flex flex-col flex-1 min-w-0 justify-between h-full">
          <div className="mb-4 sm:mb-6">
            <h3 className={`font-semibold font-sans text-slate-900 mb-1.5 truncate ${isCore ? "text-lg" : "text-base"}`}>{title}</h3>
            <p className={`font-sans text-[#555555] leading-relaxed line-clamp-2 sm:line-clamp-none ${isCore ? "text-[15px]" : "text-[14px]"}`}>{description}</p>
